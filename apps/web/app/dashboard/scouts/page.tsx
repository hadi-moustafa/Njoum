...
import ScoutsCreate from './ScoutsCreate';
import DeleteScoutItem from './DeleteScoutItem';
...
export default async function ScoutsPage({ searchParams }: { searchParams: { tab?: string } }) {
  ...
  const badgeOptions = badges.map((b: any) => ({ id: b.id, name: b.name }));
  ...
  return (
    <div>
      ...
      {tab === 'troops' && (
        <>
          <div className="flex justify-end mb-4">
            <ScoutsCreate mode="troop" label="فوج جديد" />
          </div>
              ...
              <div key={t.id} className="bg-white rounded-2xl border border-njoum-border p-5">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <DeleteScoutItem id={t.id} type="troop" />
                  </div>
                </div>
      ...
      {tab === 'activities' && (
        <>
          <div className="flex justify-end mb-4">
            <ScoutsCreate mode="activity" label="نشاط جديد" badgeIds={badgeOptions} />
          </div>
              ...
              <tbody className="divide-y divide-njoum-border">
                {(activities ?? []).map((a: any) => (
                  <tr key={a.id} className="hover:bg-njoum-bg/40 transition-colors">
                    ...
                    <td className="px-6 py-3">
                      <DeleteScoutItem id={a.id} type="activity" />
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {tab === 'badges' && (
        <>
          <div className="flex justify-end mb-4">
            <ScoutsCreate mode="badge" label="شارة جديدة" />
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {(badges ?? []).map((b: any) => (
              <div key={b.id} className="bg-white rounded-2xl border border-njoum-border p-5 text-center hover:border-primary/40 transition-colors relative">
                <div className="absolute top-3 left-3">
                  <DeleteScoutItem id={b.id} type="badge" />
                </div>
